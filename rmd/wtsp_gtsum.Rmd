...
table_wtsp_summary <- 
  ...
  tbl_summary(by=Fgen,
      ...
      P4Mass ~ "Mass (mg)",
      P4Length ~ "Length (mm)")) %>%
  modify_caption("Table 1. Summary of White-throated Sparrow P4 characteristics") %>% # Adding a table header
  modify_header(
    update = list(
      label ~ '',
...
```
```{r}
feather_boxplot <- ggplot(feather_clean, aes(x = Age, y = Wing, fill = Age)) +
  geom_boxplot() +
  theme_bw() +
  xlab("Age")+
...
For the ANOVA we will group by age categories for the purpose of this assignment. These age categories may not represent an ecologically significant effect as HY birds become classified as SY birds after January 1.
